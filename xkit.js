...
(function() {
	...
	XKit = {
		...
		tools: {
			 ...
			 */
			Nx_XHR: details => new Promise(async (resolve, reject) => {
			 ...
			 */
			cartesian_product: (items, current = []) => {
		...
		interface: {
			...
			kitty: {
				 ...
				 */
				get: async function(callback, retry_mode = false) {
			...
			post_window: {
				 ...
				 */
				blog: () => $("#channel_id").val() || $(".post-form--header [data-js-tumbleloglabel]").text(),

				reblogging_from: () => $(".post-form--header .reblog_source .reblog_name").text(),
			 ...
			 */
			translate: key => new Promise(resolve => {
			 ...
			 */
			mass_edit: function(post_ids, config) {
				const path = {
			...
			hide: function(selector, extension) {
	...
	/**
	 * Migrated from XKit Patches for v7.9.2
	 */
